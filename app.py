...
def exportar_pdf():
    ...
    tabela.setStyle(TableStyle([
        ...
        ('FONTSIZE', (0,0), (-1,-1), 10),
        ('ROWBACKGROUNDS', (0,1), (-1,-1), [colors.HexColor('#FFC7CE'), colors.HexColor('#FFC7CE')]),
    ...
    for i, row in enumerate(df.itertuples(), start=1):
        if row.nivel == 'ALTO':
            cor = colors.HexColor('#FFC7CE')
        elif row.nivel == 'MODERADO':
            cor = colors.HexColor('#FFEB9C')
        else:
            cor = colors.HexColor('#C6EFCE')
